...
function start(mode, type, selection) {
    if (mode == -1) {
        ...
        if (mode > 0)
            ...
            qm.dispose();
        if (status == 0)
            qm.sendAcceptDecline("Ola, #h0#. Bem vindo ao Mundo Maple. It's currently event season, and we're welcome new characters with a gift. Would you like your gift now?");
        else if (status == 1) {
            qm.forceStartQuest();
