...
function enter(pi) {
    if (pi.getPlayer().getMap().getReactorByName("door").getState() == 1) {
        if ((pi.getPlayer().getMapId() / 100) % 100 != 38) {
            ...
            pi.getPlayer().message("Voce recebeu " + pi.getPlayer().addDojoPointsByMap() + " pontos de treinamento. Agora voce acumulou " + pi.getPlayer().getDojoPoints() + " pontos.");
        ...
        pi.getPlayer().message("A porta ainda nao esta aberta.");
        return false;
    }
}
