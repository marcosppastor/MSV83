...
function enter(pi) {
    ...
    if (papuMap.getCharacters().size() == 0) {
        pi.getPlayer().dropMessage("A sala esta livre. Uma oportunidade perfeita para enfrentar o Papulatus.");
        papuMap.resetReactors();
    } else { // someone is inside
        for (var i = 0; i < 3; i++) {
            if (papuMap.getMonsterById(8500000 + i) != null) {
                pi.getPlayer().dropMessage("Alguem ja esta enfrentando o Papulatus.");
                return false;
            }
        }
    }
