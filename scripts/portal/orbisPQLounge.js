...
function enter(pi) {
	...
	if (playerStatus) { //Leader
		if (eim.getProperty("7stageclear") == null) {
			...
			pi.getPlayer().getClient().getSession().write(MaplePacketCreator.serverNotice(5, "Você não pode voltar nesta sala."));
			return false;
		}
	} else { //Not leader
		if (party.get(0).getMapId() == 920010600) { //Check what map the leader is in
			...
			pi.getPlayer().getClient().getSession().write(MaplePacketCreator.serverNotice(5, "Você não pode entrar nesta sala se o seu líder não estiver nela."));
			return false;
		}
	}
}