...
function playerRevive(eim,player) {
	...
	if (party.size() < minPlayers) {
		end(eim,"Não há suficientes jogadores restantes, a batalha acabou.");
	}
	return false;
}
...
function playerDisconnected(eim,player) {
	...
	if (player.getName().equals(eim.getProperty("leader"))) {
		...
		while (iter.hasNext()) {
			var pl = iter.next();
			pl.getClient().getSession().write(Packages.tools.MaplePacketCreator.serverNotice(6,"O lider da expedição se desconectou."));
		}
	}
...
function monsterValue(eim,mobId) { // potentially display time of death? does not seem to work
	if (mobId == 8800002) { // 3rd body
		...
		while (iter.hasNext()) {
			var pl = iter.next();
			pl.getClient().getSession().write(Packages.tools.MaplePacketCreator.serverNotice(6,"Parabéns por derrotar Zakum!"));
		}
	}
	return -1;
}
...
function playerExit(eim,player) {
	...
	player.changeMap(exitMap,exitMap.getPortal(0));
        ...
        if (party.size() < minPlayers) { //not enough after someone left
                end(eim,"Já não há jogadores suficientes para continuar, e os restantes devem ser removidos.");
        }
}
