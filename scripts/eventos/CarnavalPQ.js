...
function setup(eim) {
	em.setProperty("CPQOpen", "false")
	var instanceName = "CarnavalPQ" + instanceId;
	var eim = em.newInstance(instanceName);
	eim.setProperty("stage", "0");
	instanceId = em.getChannelServer().getInstanceId();
	exitMap = em.getChannelServer().getMapFactory().getMap(100000000);
    flagmap = em.getChannelServer().getMapFactory().getMap(924010000 ); // Mapa de início para PQ + Instância.
	exitMap2 = em.getChannelServer().getMapFactory().getMap(100000000);
	var mf = eim.getMapFactory();
	em.getChannelServer().addInstanceId();
	var map = mf.getMap(924010000 ); // Mapa de início para PQ + Instância.
	map.removePortals();
...
function playerEntry(eim, player) {
	var map = eim.getMapInstance(924010000 ); // Mapa de início para PQ + Instância.
	player.changeMap(map, map.getPortal(0));
	map.broadcastMessage(Packages.tools.MaplePacketCreator.serverNotice(6, "[Halloween] Derrote o monstro e retire dele o item necessário,então fale com o NPC!"));
...
function monsterSpawn(eim, player) {
    var mf = eim.getMapFactory();
	var bossmap = mf.getMap(924010000); // Mapa de início para PQ + Instância.
	bossmap.removePortals();
	bossmap.killAllMonsters(false);
	var mob = Packages.server.life.MapleLifeFactory.getMonster(9500195  );
	var overrideStats = new Packages.server.life.MapleMonsterStats();
	overrideStats.setHp(500);
	overrideStats.setExp(25000);
	overrideStats.setMp(mob.getMaxMp());
        //overrideStats.setDrop(4001302);
	mob.setOverrideStats(overrideStats);
	mob.setHp(overrideStats.getHp());
        eim.registerMonster(mob);
	bossmap.spawnMonsterOnGroudBelow(mob, new java.awt.Point(-8,111));
      	bossmap.spawnNpc(2082015, new java.awt.Point(-5,26));//Monóculo
