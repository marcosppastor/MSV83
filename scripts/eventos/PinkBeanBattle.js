...
function setup(eim) {
	exitMap = em.getChannelServer().getMapFactory().getMap(270050000 );
	if (exitMap == null) 
...
function playerEntry(eim,player) {
	var map = eim.getMapInstance(270050100); // Last Mission: Zakum's Altar
...
function monsterValue(eim,mobId) { // potentially display time of death? does not seem to work
	if (mobId == 8820001) { // 3rd body
		var party = eim.getPlayers();
		var iter = party.iterator();
		while (iter.hasNext()) {
			var pl = iter.next();
			pl.getClient().getSession().write(Packages.tools.MaplePacketCreator.serverNotice(6,"Parabéns por derrotar o super PinkBean!"));
		}
	}
	return -1;
}
