...
function setup(eim) {
	em.setProperty("PaisPQFacilOpen", "false")
	var instanceName = "PaisPQFacil" + instanceId;
	var eim = em.newInstance(instanceName);
	eim.setProperty("stage", "0");
	instanceId = em.getChannelServer().getInstanceId();
	exitMap = em.getChannelServer().getMapFactory().getMap(100000000);
    flagmap = em.getChannelServer().getMapFactory().getMap(683000000); // Mapa de início para PQ + Instância.
	exitMap2 = em.getChannelServer().getMapFactory().getMap(100000000);
	var mf = eim.getMapFactory();
	em.getChannelServer().addInstanceId();
	var map = mf.getMap(683000000); // Mapa de início para PQ + Instância.
	map.removePortals();
...
function playerEntry(eim, player) {
	var map = eim.getMapInstance(683000000); // Mapa de início para PQ + Instância.
...
function monsterSpawn(eim, player) {
    var mf = eim.getMapFactory();
	var bossmap = mf.getMap(683000000); // Mapa de início para PQ + Instância.
	bossmap.removePortals();
	bossmap.killAllMonsters(false);
	var mob = Packages.server.life.MapleLifeFactory.getMonster(9400603);
	...
	bossmap.spawnMonsterOnGroudBelow(mob, new java.awt.Point(-9,209));
      	bossmap.spawnNpc(2010000 , new java.awt.Point(-470,-55));//Monóculo
