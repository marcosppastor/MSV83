...
function setup(eim) {
	em.setProperty("TruePQOpen", "false")
	var instanceName = "TruePQ" + instanceId;
	var eim = em.newInstance(instanceName);
	eim.setProperty("stage", "0");
	instanceId = em.getChannelServer().getInstanceId();
	exitMap = em.getChannelServer().getMapFactory().getMap(100000000);
    flagmap = em.getChannelServer().getMapFactory().getMap(677000011); // Mapa de início para PQ + Instância.
	exitMap2 = em.getChannelServer().getMapFactory().getMap(100000000);
	var mf = eim.getMapFactory();
	em.getChannelServer().addInstanceId();
	var map = mf.getMap(677000011); // Mapa de início para PQ + Instância.
	map.removePortals();
...
function playerEntry(eim, player) {
	var map = eim.getMapInstance(677000011); // Mapa de início para PQ + Instância.
	...
	eim.schedule("monsterSpawn", 500);
	player.getClient().getSession().write(Packages.tools.MaplePacketCreator.getClock(1600));
...
function monsterSpawn(eim, player) {
    var mf = eim.getMapFactory();
	var bossmap = mf.getMap(677000011); // Mapa de início para PQ + Instância.
	bossmap.removePortals();
	bossmap.killAllMonsters(true);
