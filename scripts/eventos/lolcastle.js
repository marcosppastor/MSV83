...
function announce() {
	...
	em.getChannelServer().broadcastPacket( 
		Packages.tools.MaplePacketCreator.serverNotice(6, "[Evento] Campo de Julgamento sera aberto em " + i + " minutos"));
	i--;
}
...
function mesoDistribution() {
	...
	while (iter.hasNext()) {
		...
		if (eim.getPlayerCount() > 0) {
			...
			map.broadcastMessage(Packages.tools.MaplePacketCreator.serverNotice(6, "[Evento] " + winner.getName() + " ganhou " + meso + " mesos"));
...
function start() {
	scheduleNew();
	em.getChannelServer().broadcastPacket( 
		Packages.tools.MaplePacketCreator.serverNotice(6, "[Evento] O Campo de Julgamento esta fechado para novos participantes."));	
	var iter = em.getInstances().iterator();
	while (iter.hasNext()) {
		var eim = iter.next();
		if (eim.getPlayerCount() > 0) {
...
function playerDead(eim, player) {
	...
	player.getClient().getSession().write(Packages.tools.MaplePacketCreator.serverNotice(1, "Voce perdeu esta rodada tendo " + eim.getKillCount(player) + " pontos."));
}
...
function allMonstersDead(eim, player) {
	...
	while (iter.hasNext()) {
		...
		winner.getClient().getSession().write(Packages.tools.MaplePacketCreator.serverNotice(1, "Voce ganhou esta rodada com " + eim.getKillCount(winner) + " pontos. Aguarde 2 minutos para ser levado daqui."));
