...
function setup(eim) {
	em.setProperty("PaisPQMediaOpen", "false")
	var instanceName = "PaisPQMedia" + instanceId;
	var eim = em.newInstance(instanceName);
	eim.setProperty("stage", "0");
	instanceId = em.getChannelServer().getInstanceId();
	exitMap = em.getChannelServer().getMapFactory().getMap(100000000);
    flagmap = em.getChannelServer().getMapFactory().getMap(970000004); // Mapa de início para PQ + Instância.
	exitMap2 = em.getChannelServer().getMapFactory().getMap(100000000);
	var mf = eim.getMapFactory();
	em.getChannelServer().addInstanceId();
	var map = mf.getMap(970000004); // Mapa de início para PQ + Instância.
	map.removePortals();
...
function playerEntry(eim, player) {
	var map = eim.getMapInstance(970000004); // Mapa de início para PQ + Instância.
...
function monsterSpawn(eim, player) {
    var mf = eim.getMapFactory();
	var bossmap = mf.getMap(970000004); // Mapa de início para PQ + Instância.
	bossmap.removePortals();
	bossmap.killAllMonsters(false);
	var mob = Packages.server.life.MapleLifeFactory.getMonster(9300285);
	...
	bossmap.spawnMonsterOnGroudBelow(mob, new java.awt.Point(21,83));
      	bossmap.spawnNpc(2010000, new java.awt.Point(-166,22));//Monóculo
