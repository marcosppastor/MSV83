 ...
 * 
 */
var nomeServer = "True MapleStory";
...
function setup(eim) {
    ...
    //eim.getMapInstance(990000900).shuffleReactors();
...
function begin(eim) {
    ...
    while (iter.hasNext()) {
        iter.next().dropMessage(6,"A Missão de Clã acabou de começar.");
    }
...
function playerDisconnected(eim, player) {
    ...
    if (player.getName().equals(eim.getProperty("leader"))) { //check for party leader
        ...
        while (iter.hasNext()) {
            var pl = iter.next();
            pl.dropMessage(6,"O líder da missão se desconectou e os demais jogadores serão expulsos!.");
            if (pl.equals(player)) {
                removePlayer(eim, pl);
            }
            else {
    ...
    /*else { //boot d/ced player and check if enough players left
        ...
        if (party.size() < 6) { //five after player booted
            end(eim,"Já não há jogadores suficientes para continuar, e os restantes serão expulsos.");
        }
    }
    */
}
...
function earringcheck(eim, player) {
	var iter = eim.getPlayers().iterator();
		...
		var pl = iter.next();
			...
			pl.addHP(-30000);
			pl.getMap().broadcastMessage(MaplePacketCreator.serverNotice(6,"[" + nomeServer + " GPQ] " + pl.getName() + " morreu por não estar usando os brincos!"));
                }
        }
        eim.schedule("earringcheck", 50);
}
