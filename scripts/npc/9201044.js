		...
		if (curMap == 1) { // First Stage.
			if (playerStatus) { // party leader
				if (status == 0) {
					...
					if (preamble == null) {
						cm.sendNext("Olá e bem-vindo ao primeiro estágio. Vou liberr 3 portais aqui, e você decidirá qual deles é o certo, as meninas irão no portal esquerdo, enquanto os rapazes vão para o portal certo. Mate os monstros lá e depois peça o líder para conversar com meu amigo Glimmer Man.");
		...
		if (curMap == 2) {
	                        	...
	                        	// check for people on ropes
	                        	        ...
	                        	        if (totplayers == 5 /*|| debug*/) {
	                        	                ...
	                        	                if (testcombo || debug) {
														...
														if (tries == 7) {
															cm.sendOk("Você tentou a combinação sete vezes, então agora será reiniciado.");
															var sequenceNum = Math.floor(Math.random() * curcombo.length);
															eim.setProperty("stage" + nthtext + "combo",sequenceNum.toString());
	                        	                ...
	                        	                if (debug) {
													...
													cm.sendNext("Parece que você ainda não encontrou as cordas corretas. Continue tentando!!");
													//var combo = curcombo[parseInt(eim.getProperty("stage" + nthtext + "combo"))];
													//cm.sendOk(combo);
													//clear(curMap,eim,cm);
													cm.dispose();
	        ...
	        if (playerStatus) { // leader
	                if (status == 0) {
	                        ...
	                        preamble = eim.getProperty("leader" + nthtext + "preamble");
	                        if (preamble == null) {
	                                cm.sendNext("Oi. Bem-vindo ao terceiro estágio. Ao lado de mim, existem nove plataformas. Tudo o que você tem a fazer, é ter 5 pessoas em pé e então, o líder deve clicar em mim para ver se está correto. Se for #b errada #k correto, um número de #e Slimes #n irá aparecer e esse número é a quantidade de plataformas corretas nas quais você está. Além disso, você só tem 7 suposições antes da reinicialização da combinação.");
	                                eim.setProperty("leader" + nthtext + "preamble","done");
	                                var sequenceNum = Math.floor(Math.random() * curcombo.length);
	                                eim.setProperty("stage" + nthtext + "combo",sequenceNum.toString());
	                        	...
	                        	// check for people on ropes
	                        	        ...
	                        	        if (totplayers == 5 /*|| debug*/) {
	                        	                ...
	                        	                if (testcombo || debug) {
														...
														if (tries == 7) {
															cm.sendOk("A combinação foi reiniciada porque você não conseguiu direito em 7 suposições.");
															var sequenceNum = Math.floor(Math.random() * curcombo.length);
															eim.setProperty("stage" + nthtext + "combo",sequenceNum.toString());
											...
											if (debug) {
												var outstring = "Os objetos contêm:"
												for (i = 0; i < objset.length; i++) {
													outstring += "\r\n" + (i+1).toString() + ". " + objset[i].toString();
												}
												cm.sendNext(outstring); 
											...
											cm.sendNext("Parece que você ainda não encontrou as 5 "+ nthobj +". Por favor, pense em uma combinação diferente de "+ nthobj +". Apenas 5 são permitidos para "+ nthverb +" nas "+ nthobj +", e se você "+ nthpos +" não pode contar como uma resposta, por favor, tenha em mente isso. Continue!");
											cm.dispose();
	                        	        } 
	                        	}
	                        }
