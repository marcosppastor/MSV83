		...
		if (mode == 1)
			...
			if (playerStatus) { // party leader
				if (status == 0) {
					...
					if (preamble == null) {
						cm.sendNext("Ola e bem vindo ao setimo estagio da Missao de Ludibrium.\r\nNeste mapa havera alguns monstros. Destrua-os e me colete #r3#k cupons.\r\nApos coletar a quantia exigida, fale comigo pois liberarei o portal para que voce possa seguir em frente.");
						...
						cm.dispose();
							...
							if (cm.haveItem(4001022, 3) == false) {
								cm.sendNext("Me desculpe, mas voce nao coletou os 3 cupons exigidos para prosseguir com a missao.");
								cm.dispose();
							}
							else {
								cm.sendNext("#bParabens#k por concluir o setimo estagio!\r\nVou abrir o portal agora.");
								clear(1,eim,cm);
