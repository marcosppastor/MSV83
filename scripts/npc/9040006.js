                ...
                if (eim.getProperty("leader").equals(cm.getPlayer().getName())) {
                        if (cm.getPlayer().getMap().getReactorByName("watergate").getState() > 0){
                                ...
                                if (currentCombo == null || currentCombo.equals("reset")) {
                                        ...
                                        cm.sendOk("Esta fonte guarda a passagem secreta para a sala do trono. Você ira oferecer items nesta area para prosseguir. Os vassalos devem dizer se as suas ofertas são aceitas, e se não, quais vassalos estão descontentes. Você tem sete tentativas!")
                                } else {
                                        ...
                                        if (guess != null) {
                                                if (combo == guess) {
                                                        ...
                                                        cm.sendOk("Você pode prosseguir.");
                                                        ...
                                                        if (attempt < 7) {
                                                                ...
                                                                if (results[0] != 0) {
                                                                        if (results[0] == 1) {
                                                                                ...
                                                                                string += results[0] + " vassalos estão #esatisfeitos#n com suas ofertas.\r\n";
                                                                        }
                                                                }
                                                                if (results[1] != 0) {
                                                                        if (results[1] == 1) {
                                                                                ...
                                                                                string += results[1] + " vassalos estão #einsatisfeitos#n com suas ofertas.\r\n";
                                                                        }
                                                                }
                                                                if (results[2] != 0) {
                                                                        if (results[2] == 1) {
                                                                ...
                                                                cm.sendOk("Você falhou no teste. Por favor, tente novamente mais tarde.");
                        ...
                        cm.sendOk("Peça para o lider falar comigo.");
                }
        }
        cm.dispose();
}
...
function getGroundItems() {
    ...
    if (items.size() != 4) {
	cm.getPlayer().dropMessage("Verifique se a sua tentativa está devidamente ajustada na frente das estatuas.");
	return null;
    }
	...
	if (id < 4001027 || id > 4001030) {
	    cm.getPlayer().dropMessage("Alguns item do mapa não fazem parte dos 4 items necessários.");
	    return null;
	} else {
	    //check item location
	    for (var i = 0; i < 4; i++) {
	...
	cm.getPlayer().dropMessage("Por favor, faça a posição correta: " + (itemInArea[0] == -1 ? "Estatua 1, " : "") + (itemInArea[1] == -1 ? "Estatua 2, " : "") + (itemInArea[2] == -1 ? "Estatua 3, " : "") + (itemInArea[3] == -1 ? "Estatua 4. " : ""));
              /*  for (var i = 0; i < 4; i++) {
