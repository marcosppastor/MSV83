...
function action(mode, type, selection) {
    if (mode == -1) {
        ...
        if (mode == 1)
            ...
            status--;
        if (status == 0) {
            cm.sendSimple("Ei, ai ~! Eu sou J.J.! Estou no comando das lentes cosméticas aqui na NLC Shop! Se você tem um #b#t5152036##k, Posso obter as melhores lentes de cosméticos que você já teve! Agora, o que você gostaria de fazer??\r\n#L1#Eu quero comprar um #b#t5152036##k for " + price + " mesos, por favor!#l\r\n\#L2#Eu já tenho um cupom!#l");
        } else if (status == 1) {
            if (selection == 1) {
                if(cm.getMeso() >= price) {
                    cm.gainMeso(-price);
