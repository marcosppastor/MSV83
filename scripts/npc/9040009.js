			...
			status--;
                ...
                if (eim == null) {
                        ...
                        if (eim.getProperty("leader").equals(cm.getPlayer().getName())) {
                                if (cm.getPlayer().getMap().getReactorByName("statuegate").getState() > 0){
                                        ...
                                        if (status == 0) {
                                                if (eim.getProperty("stage1status") == null || eim.getProperty("stage1status").equals("waiting")) {
                                                        ...
                                                        if (stage == 1) {
                                                                cm.sendOk("Neste desafio, vou mostrar-lhes um padrão nas estatuas em torno de mim. Quando eu der a ordem, repita o padrão para poder continuar.");
                                                        }
                                                        else {
                                                                cm.sendOk("Vou agora apresentar um quebra-cabeça mais dificil para você.")
                                                        }
                                                }
                                                        ...
                                                        if (eim.getProperty("stage1combo").equals(eim.getProperty("stage1guess"))) {
                                                                if (stage == 3) {
                                                                        ...
                                                                        cm.sendOk("Excelente trabalho. Por favor, prossiga para a próxima fase.");
                                                                        ...
                                                                        cm.sendOk("Muito bom. Você ainda tem mais trabalho a concluir. Fale comigo novamente quando estiver pronto.");
                                                                        eim.setProperty("stage1phase", stage + 1);
                                                                        cm.mapMessage("Você concluiu a parte (" + stage + ") Teste do Gatekeeper.");
                                                                }
                                                                
                                                        } else {
                                                                cm.sendOk("Você falhou neste teste.");
                                                                cm.mapMessage("Você falhou no Teste do Guardião.");
                                                ...
                                                cm.playerMessage(reactorString);*/
                                                cm.mapMessage("Por favor, aguarde enquanto a combinação e revelada.");
                                ...
                                cm.sendOk("Eu preciso que o lider de seu grupo fale comigo, mais ninguém.");
                                cm.dispose();
                        }
                }
        }
