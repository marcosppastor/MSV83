    ...
    if (mode == -1) {
        ...
        if (status == 0) {
            if (cm.getPlayer().getEventInstance().getProperty("leader").equals(cm.getPlayer().getName())) {
                ...
                {
                    cm.sendOk("Depois do que pensei que seria um sono imortal, finalmente encontrei alguém que salvará Sharenian. Eu realmente posso descansar em paz agora.");
                    cm.dispose();
                }
                else {
                    var prev = cm.getPlayer().getEventInstance().setProperty("stage4clear","true",true);
                    if (prev == null) {
                        cm.sendNext("Depois do que pensei que seria um sono imortal, finalmente encontrei alguém que salvará Sharenian. Este velho agora abrirá o caminho para você terminar a missão." + mode);
                    }
            ...
            else
            {
                if (cm.getPlayer().getEventInstance().getProperty("stage4clear") != null && cm.getPlayer().getEventInstance().getProperty("stage4clear").equals("true"))
                    ...
                    cm.sendOk("Preciso que o líder da sua festa fale comigo, ninguém mais.");
                cm.dispose();
            }
        }
        else if (status == 1) {
