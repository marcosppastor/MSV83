...
function start() {
    cm.sendNext("Oi #h #, eu sou o #bJean#k. Estou esperando pelo meu irmao #bPaul#k. Aparentemente, era para ele estar aqui agora...");
}
...
function action(mode, type, selection) {
    if (mode == -1) {
        ...
        if (mode == 1)
            ...
            status--;
        if (status == 1) {
            cm.sendNextPrev("Hmm... Voce sabe dos boatos? Estao dizendo que quando se inicia um evento, muita pessoas resolvem participaar...");
        } else if (status == 2) {
            cm.sendSimple("#h #... Porque nao falarmos sobre os eventos, enquanto espero pelo meu irmao?\r\nO que voce deseja saber/fazer?\r\n#L0##e1.#n#b O que e um evento?#k#l\r\n#L1##e2.#n#b Explique-me sobre eventos.#k#l\r\n#L2##e3.#n#b Estou pronto para participar de um evento!#k#l");
        } else if (status == 3) {
            if (selection == 0) {
                cm.sendNext("Um evento e basicamente uma comemoracao! Geralmente comemoram-se novos jogadores, conquistas ou fazem premiacoes para ajudar nos, jogadores.");
                cm.dispose();
            } else if (selection == 1) {
                cm.sendSimple("Existem muitos eventos... Escolha um para que eu possa explicar! #b\r\n#L0# Ola Ola#l\r\n#L1# MapleStory Maple Physical Fitness Test#l\r\n#L2# Snow Ball#l\r\n#L3# Coconut Harvest#l\r\n#L4# OX Quiz#l\r\n#L5# Treasure Hunt#l#k");
            } else if (selection == 2) {
				if (cm.getEvent() != null && cm.getEvent().getLimit() > 0) {
					cm.getPlayer().saveLocation("EVENTO");
					...
					cm.sendNext("#h #, ou nao ha eventos disponiveis, ou voce ja participou de algum dentre as ultimas 24 horas.\r\nPor favor, tente novamente mais tarde.");
					cm.dispose();                
            }
			}
        } else if (status == 4) {
