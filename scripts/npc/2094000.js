		...
		if (status == 0) {
			...
			if (!cm.isLeader()) { // not party leader
				...
				if (next) {
					...
					cm.sendOk("Seu grupo não é um grupo de ao menos de quatro pessoas. Certifique-se de que todos os seus membros estão presentes e que são qualificados para participar dessa missão. Eu vejo #b" + levelValid.toString() + " #kOs membros estão no nível certo, e #b" + inMap.toString() + "#k Estão no meu mapa. Se isso estiver errado, #b Relogue  e faça o login novamente, # k ou refaça o grupo.");
					cm.dispose();
				}
			}
		}
