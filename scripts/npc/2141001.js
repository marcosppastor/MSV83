...
function action(mode, type, selection) {
    if (mode == -1) {
        ...
        if (status == 0) {
			...
			cm.sendOk("Existe um requisito de nível superior a 120 para desafiar o  Pink Bean.");
			cm.dispose();
			return;
		}
		if (cm.getPlayer().getClient().getChannel() !=3) {
			cm.sendOk("Pink Bean só pode ser desafiado no canal 3.");
			cm.dispose();
			return;
		}
            if (cm.getSquadState(MapleSquadType.PINKBEAN) == 0) {
                cm.sendSimple("Você e seu time desejam enfrentar o boss mais forte do maplestory? #rPinkBean#k?\r\n#b#L1#Sim, desejamos!#l\r\n\#L2#Nao. Irei esperar uns minutos...#l");
            } else if (cm.getSquadState(MapleSquadType.PINKBEAN) == 1) {
                if (cm.checkSquadLeader(MapleSquadType.PINKBEAN)) {
                    cm.sendSimple("O que você gostaria de fazer?#b\r\n\r\n#L1#Ver a lista de participantes#l\r\n#L2#Fechar os registros de entrada#l\r\n#L3#Enfrentar o #ePinkBean#n!#l");
                    status = 19;
                } else if (cm.isSquadMember(MapleSquadType.PINKBEAN)) {
                    var noOfChars = cm.numSquadMembers(MapleSquadType.PINKBEAN);
                    var toSend = "Os seguintes jogadores estao participando para enfrentar o #ePinkBean#n:\r\n#b";
                    for (var i = 1; i <= noOfChars; i++) {
                        toSend += "\r\n#L" + i + "#" + cm.getSquadMember(MapleSquadType.PINKBEAN, i - 1).getName() + "#l";
                    }
                    cm.sendSimple(toSend);
                    cm.dispose();
                } else {
                    cm.sendSimple("Voce e seu time desejam enfrentar o tenebroso #rPinkBean#k?\r\n#b#L1#Sim, desejamos!#l\r\n\#L2#Nao. Irei esperar uns minutos...#l");
                    status = 9;
                }
            } else {
                if (cm.checkSquadLeader(MapleSquadType.PINKBEAN)) {
            ...
            if (selection == 1) {
                ...
                cm.sendOk("Certo. escolha sua....");
                cm.dispose();
            }
        } else if (status == 10) {
            if (selection == 1) {
                if (cm.numSquadMembers(MapleSquadType.PINKBEAN) > 29) {
                    cm.sendOk("Lamento, mas ja existem 29 participantes para enfrentar o PinkBean..");
                    cm.dispose();
                } else {
                    if (cm.canAddSquadMember(MapleSquadType.PINKBEAN)) {
                        cm.addSquadMember(MapleSquadType.PINKBEAN);
                        cm.sendOk("Aguarde as instruções do Lider do seu grupo..");
                        cm.dispose();
                    } else {
                        cm.sendOk("Lamento, mas o Lider ainda nao 'esta pronto' para enfrentar o #ePinkBean#n");
                        cm.dispose();
                    }
                }
            } else if (selection == 2) {
            ...
            if (selection == 1) {
                var noOfChars = cm.numSquadMembers(MapleSquadType.PINKBEAN);
                var toSend = "Os seguintes jogadores estão participando da expedição para enfrentar o #ePinkBean#n:\r\n#b";
                for (var i = 1; i <= noOfChars; i++) {
                    if (i == 1)
                        toSend += "\r\n" + cm.getSquadMember(MapleSquadType.PINKBEAN, i - 1).getName();
                    else
                        toSend += "\r\n#L" + i + "#" + cm.getSquadMember(MapleSquadType.PINKBEAN, i - 1).getName() + "#l";
                ...
                if (cm.getSquadState(MapleSquadType.PINKBEAN) == 1) {
                    cm.setSquadState(MapleSquadType.PINKBEAN, 2);
                    cm.sendOk("Novos registros para enfrentar o #ePinkBean#n foram fechados. Para abrir novamente, fale comigo.");
                } else {
                    cm.setSquadState(MapleSquadType.PINKBEAN, 1);
                    cm.sendOk("Novos registros para enfrentar o #ePinkBean#n foram abertos. Para fechar novamente, fale comigo.");
                }
                cm.dispose();
            } else if (selection == 3) {
                ...
                cm.setSquadState(MapleSquadType.PINKBEAN, 2);
                cm.sendOk("Agora sim voces estao prontos para enfrentar o #ePinkBean#n.");
                //cm.warpSquadMembers(MapleSquadType.HORNTAIL, 270050100 );
            ...
            if (em == null) {
                ...
                // Begin the PQ.
                em.startInstance(cm.getSquad(MapleSquadType.PINKBEAN), cm.getChar().getMap());
            }
            cm.dispose();
				//cm.mapMessage(6, "Quebre o cristal para fazer o Horntail emergir!");
				cm.mapMessage(6, "Caso o lider esteja pronto para iniciar a expedição fale comigo ~~ Temple Keeper.");
				cm.getPlayer().getMap().broadcastMessage(Packages.tools.MaplePacketCreator.serverNotice(0, cm.getPlayer().getName() + " iniciou uma batalha contra o PinkBean, estando no canal" + cm.getPlayer().getClient().getChannel() + "."));
                status = 29;
            cm.dispose();
                    }
            ...
            if (selection > 0) {
                cm.removeSquadMember(MapleSquadType.PINKBEAN, selection - 1, true);
...
function action(mode, type, selection) {
    if (mode == -1)
		...
		if (cm.getPlayer().getClient().getChannel() !=5) {
            ...
            //	return;
            //  }
            cm.sendSimple("A batalha contra o PinkBean comeca aqui!\r\nO que voce gostaria de fazer?#b\r\n#L0#Iniciar a Batalha contra o PinkBean#l\r\n#L1#Entrar em um Grupo que esteja lutando contra o PinkBean#l");
        }
        else if (status == 1) {
            state = selection;
            if (selection == 0) {
                ...
                cm.sendGetText("Para ir ate o PinkBean, voce precisara escolher um nome para a instancia da sua batalha. Esta sera basicamente a senha de acesso para que outras pessoas possam lhe ajudar, entrando em sua batalha por meio da senha.");
		                    } else {
            ...
            if (em == null)
                ...
                if (state == 0) { // Leader
                    if (getEimForString(em,passwd) != null)
                        ...
                        var eim = em.newInstance("Pink" + passwd);
                ...
                if (state == 1) {
                    ...
                    if (eim == null)
                        cm.sendOk("Não ha nenhuma batalha registrada com este nome.");
					    //cm.dispose();
                    else {
