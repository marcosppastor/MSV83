...
function action(mode, type, selection) {
    if (mode == 1)
        ...
        cm.dispose();
    if (status == 0) {
        var selStr = "Sim, eu possuo essa forja. Se você está disposto a pagar, posso oferecer-lhe alguns dos meus serviços.#b"
        var options = new Array("Refinar um minério ","Refinar uma joia","Eu tenho casco de metal do porco de ferro ...","Atualize uma garra");
        ...
        if (selectedType == 0){ //mineral refine
            var selStr = "Então, que tipo de minério mineral você gostaria de refinar??#b";
            ...
            var selStr = "Você sabe disso? Muitas pessoas não percebem o potencial no casaco de metal do Iron Hog ... Eu posso fazer isso em algo especial, se você quiser.";
            equip = false;
            cm.sendYesNo(selStr);
        }
        else if (selectedType == 3){ //claw refine
            var selStr = "Ah, você deseja atualizar uma garra? Então me diga, qual deles?#b";
        ...
        var prompt = "Então, você quer que eu faça alguns #t" + item + "#s? Nesse caso, quantos você quer que eu faça??";
        ...
        prompt += " Nesse caso, vou precisar de itens específicos de você para fazê-lo. Certifique-se de ter espaço em seu inventário, no entanto!#b";
        ...
        if (!complete)
            cm.sendOk("Não posso aceitar substitutos. Se você não tem o que eu preciso, não vou poder ajudá-lo.");
        else {
            if (mats instanceof Array) {
                for (var i = 0; i < mats.length; i++){
            ...
            cm.sendNext("Phew ... Eu quase não pensei que isso funcionaria por um segundo ... Bem, espero que você goste, de qualquer maneira.");
        }
        cm.dispose();
    }
}