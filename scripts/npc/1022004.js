...
function action(mode, type, selection) {
    if (mode == 1)
        ...
        cm.dispose();
    if (status == 0 && mode == 1) {
        var selStr = "Um ... Oi, sou o aprendiz do Sr. Trovão. Ele está subindo de idade, então ele lida com a maior parte do trabalho pesado enquanto lido com alguns dos trabalhos mais leves. O que posso fazer para você?#b"
        ...
        if (selectedType == 0){ //glove refine
            ...
            var selStr = "Modificar uma luva? Isso não deve ser muito difícil. O que você tem em mente??#b";
            ...
            var selStr = "Materiais? Conheço alguns materiais que eu posso fazer para você...#b";
            var materials = new Array ("Faça madeira processada com galho de árvore","Faça madeira processada com lenha","Faça parafusos (pacote de 15)");
	...
	var prompt = "Então, você quer fazer alguns #t" + item + "#s? neste caso, quantos deseja fazer?";
	...
	prompt += " Nesse caso, vou precisar de itens específicos de você para fazê-lo. Certifique-se de que você tenha espaço em seu inventário.!#b";
        ...
        if(!cm.canHold(item)) {
            cm.sendOk("Cheque seu inventário, há espaços livres? .");
            cm.dispose();
            return;
        }
        ...
        if (!complete)
            cm.sendOk("Ainda sou aprendiz, não sei se posso fazer outros itens ainda ... Você pode trazer o que a produção exige?");
        else {
            if (mats instanceof Array) {
                for (var i = 0; i < mats.length; i++){
            ...
            cm.sendOk("Isso foi certo? Venha até mim novamente se você tiver alguma coisa para que pratique.");
        }
        cm.dispose();
    }
}