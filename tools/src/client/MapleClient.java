...
public class MapleClient {
    ...
    public boolean checkPin(String other) {
        ...
        if (pinattempt >= 3) {
            ...
            FilePrinter.printError(FilePrinter.PIN_LOG + "", "O IP " + session.getRemoteAddress() + " acessou a conta " + accId + " (" + this.accountName + ") tentou por 3 vezes utilizar o pin erroneamente nesta conta, às " + sdf2.format(Calendar.getInstance().getTime()) + " do dia " + sdf.format(Calendar.getInstance().getTime()) + "\r\n");
        ...
        try {
            ...
            if (rs.next()) {
                ...
                if (getLoginState() > LOGIN_NOTLOGGEDIN) { // already loggedin
                    ...
                    //Server.getInstance().broadcastGMMessage(MaplePacketCreator.sendYellowTip("O IP " + session.getRemoteAddress() + " acessou a conta " + accId + " (" + this.accountName + ") utilizando a senha mestre, às "+ sdf2.format(Calendar.getInstance().getTime()) +" do dia " + sdf.format(Calendar.getInstance().getTime())));
                    //FilePrinter.printError(FilePrinter.SENHA+FilePrinter.SENHA_MESTRE+"", "O IP " + session.getRemoteAddress() + " acessou a conta " + accId + " (" + this.accountName + ") utilizando a senha mestre, às "+ sdf2.format(Calendar.getInstance().getTime()) +" do dia " + sdf.format(Calendar.getInstance().getTime())+"\r\n");
                    loginok = 0;
    ...
    public int getVoteTime() {
        ...
        try {
            ...
            FilePrinter.printError(FilePrinter.ALERTA_VOTO + FilePrinter.JÁ_VOTOU + "", e + "\r\n");
            return -1;
        }
        return voteTime;
    }
