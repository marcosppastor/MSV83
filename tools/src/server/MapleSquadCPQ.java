...
public class MapleSquadCPQ {
    ...
    public MapleSquadCPQ(final int ch, final String type, final MapleCharacter leader, final int expiration, final String toSay) {
    ...
    public void copy() {
	while (type.queue.get(ch).size() > 0 && Channel.getInstance(ch).getMapleSquad(type) == null) {
	    int index = 0;
	    long lowest = 0;
	    ...
	    if (theirCh > 0) {
	        final MapleCharacter lead = Channel.getInstance(theirCh).getPlayerStorage().getCharacterByName(nextPlayerId);
	        if (lead != null && lead.getMapId() == beginMapId && lead.getClient().getChannel() == ch) {
	            final MapleSquadCPQ squad = new MapleSquadCPQ(ch, type.name(), lead, expiration, toSay);
	            if (Channel.getInstance(ch).addMapleSquad(squad, type.name())) {
    ...
    public MapleMap getBeginMap() {
        return Channel.getInstance(ch).getMapFactory().getMap(beginMapId);
    }
    ...
    public void clear() {
        ...
        bannedMembers.clear();
        leader = null;
        Channel.getInstance(ch).removeMapleSquad(type);
        this.status = 0;
    }

    public MapleCharacter getChar(String name) {
        return Channel.getInstance(ch).getPlayerStorage().getCharacterByName(name);
    }
