...
public class MapleItemInformationProvider {
    ...
    protected Map<Integer, MapleInventoryType> inventoryTypeCache = new WeakHashMap<>();
    protected Map<Integer, Short> slotMaxCache = new HashMap<>();
    protected Map<Integer, MapleStatEffect> itemEffects = new WeakHashMap<>();
    protected Map<Integer, Map<String, Integer>> equipStatsCache = new WeakHashMap<>();
    ;
    protected Map<Integer, Equip> equipCache = new WeakHashMap<>();
    protected Map<Integer, Double> priceCache = new WeakHashMap<>();
    protected Map<Integer, Integer> wholePriceCache = new WeakHashMap<>();
    protected Map<Integer, Integer> projectileWatkCache = new WeakHashMap<>();
    protected Map<Integer, String> nameCache = new WeakHashMap<>();
    protected Map<Integer, String> descCache = new WeakHashMap<>();
    protected Map<Integer, String> msgCache = new WeakHashMap<>();
    protected Map<Integer, Boolean> dropRestrictionCache = new WeakHashMap<>();
    protected Map<Integer, Boolean> pickupRestrictionCache = new WeakHashMap<>();
    protected Map<Integer, Integer> getMesoCache = new WeakHashMap<>();
    private static Random rand = new Random();
    protected Map<Integer, Integer> monsterBookID = new WeakHashMap<>();
    protected Map<Integer, Boolean> onEquipUntradableCache = new WeakHashMap<>();
    protected Map<Integer, scriptedItem> scriptedItemCache = new WeakHashMap<>();
    ;
    protected Map<Integer, Boolean> karmaCache = new WeakHashMap<>();
    protected Map<Integer, Integer> triggerItemCache = new WeakHashMap<>();
    ;
    protected Map<Integer, Integer> expCache = new WeakHashMap<>();
    protected Map<Integer, Integer> levelCache = new WeakHashMap<>();
    protected Map<Integer, Pair<Integer, List<RewardItem>>> rewardCache = new WeakHashMap<>();
    ...
    protected Map<Integer, Boolean> isQuestItemCache = new WeakHashMap<>();
    protected Map<Integer, String> equipmentSlotCache = new WeakHashMap<>();
    protected final Map<Integer, ItemInformation> dataCache = new WeakHashMap<>();
    ...
    public Collection<Item> canWearEquipment(MapleCharacter chr, Collection<Item> items) {
        ...
        if (chr.getJob() == MapleJob.ADMINISTRADOR || chr.getJob() == MapleJob.SUPORTE) {
            for (Item item : items) {
    ...
    public boolean canWearEquipment(MapleCharacter chr, Equip equip, int dst) {
        ...
        if (!EquipSlot.getFromTextSlot(islot).isAllowed(dst, isCash(id))) {
            ...
            Server.getInstance().broadcastGMMessage(MaplePacketCreator.sendYellowTip("[AVISO IMPORTANTE]: " + chr.getName() + " tentou equipar o item " + itemName + " no slot " + dst + "."));
            //AutobanFactory.EDICAO_PACOTE.autoban(chr, " tentou forcar o uso de um item num slot invalido");
            //FilePrinter.printError(FilePrinter.JOGADOR_BANIDO+FilePrinter.EXPLOITS+"", "Em suma, " + chr.getName() + " tentou equipar o item " + itemName + " no seguinte slot: " + dst + ".\r\n");      	
            Server.getInstance().broadcastGMMessage(MaplePacketCreator.sendYellowTip("O jogador(a) " + chr.getName() + " acaba de ser banido(a) automaticamente por EDICAO_PACOTE (Tentou equipar o item " + itemName + " no slot " + dst + ")."));
            chr.dropMessage(0, "Você acaba de ser banido(a) automaticamente por EDICAO_PACOTE.");
            //return false;
        }
        if (chr.getJob() == MapleJob.ADMINISTRADOR || chr.getJob() == MapleJob.SUPORTE) {
            equip.wear(true);
