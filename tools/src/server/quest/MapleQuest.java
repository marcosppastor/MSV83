...
public class MapleQuest {
    ...
    protected Map<MapleQuestRequirementType, server.quest.requirements.MapleQuestRequirement> startReqs = new EnumMap<>(MapleQuestRequirementType.class);
    protected Map<MapleQuestRequirementType, server.quest.requirements.MapleQuestRequirement> completeReqs = new EnumMap<>(MapleQuestRequirementType.class);
    protected Map<MapleQuestActionType, server.quest.actions.MapleQuestAction> startActs = new EnumMap<>(MapleQuestActionType.class);
    protected Map<MapleQuestActionType, server.quest.actions.MapleQuestAction> completeActs = new EnumMap<>(MapleQuestActionType.class);
    ...
    private MapleQuest(int id) {
        ...
        if (startReqData != null) {
            for (MapleData startReq : startReqData.getChildren()) {
                ...
                server.quest.requirements.MapleQuestRequirement req = this.getRequirement(type, startReq);
		...
		MapleQuestRequirementType type = MapleQuestRequirementType.getByWZName(completeReq.getName());
                server.quest.requirements.MapleQuestRequirement req = this.getRequirement(type, completeReq);
        ...
        if (startActData != null) {
            for (MapleData startAct : startActData.getChildren()) {
                ...
                server.quest.actions.MapleQuestAction act = this.getAction(questActionType, startAct);
        ...
        if (completeActData != null) {
            for (MapleData completeAct : completeActData.getChildren()) {
                ...
                server.quest.actions.MapleQuestAction act = this.getAction(questActionType, completeAct);
    ...
    private String getIntervalTimeLeft(MapleCharacter c, IntervalRequirement r) {
        ...
        switch(mode) {
            case 2:
                ...
                str.append(hours + " horas, ");
                
            case 1:
                int minutes = (int) ((leftTime / (1000*60)) % 60);
                str.append(minutes + " minutos, ");
                
            default:
                int seconds = (int) (leftTime / 1000) % 60 ;
                str.append(seconds + " segundos");
        }
    ...
    public boolean canStart(MapleCharacter c, int npcid) {
        ...
        for (server.quest.requirements.MapleQuestRequirement r : startReqs.values()) {
            if (!r.check(c, npcid)) {
                if(r.getType().getType() == MapleQuestRequirementType.INTERVAL.getType()) {
                    c.message("Essa missão ficará disponível novamente em aproximadamente " + getIntervalTimeLeft(c, (IntervalRequirement)r) + ".");
                }
                return false;
            }
        }
    ...
    public boolean canComplete(MapleCharacter c, Integer npcid) {
        ...
        for (server.quest.requirements.MapleQuestRequirement r : completeReqs.values()) {
            if (r == null || !r.check(c, npcid)) {
                if(r.getType() == MapleQuestRequirementType.MESO) { // TODO: find a way to tell the client about the new MESO requirement type.
                    c.dropMessage(5, "Você não tem mesos suficientes para completar essa missão.");
                }
                return false;
            }
        }
    ...
    public void start(MapleCharacter c, int npc) {
        if (autoStart || canStart(c, npc)) {
            for (server.quest.actions.MapleQuestAction a : startActs.values()) {
    ...
    public void complete(MapleCharacter c, int npc, Integer selection) {
        if (autoPreComplete || canComplete(c, npc)) {
            for (server.quest.actions.MapleQuestAction a : completeActs.values()) {
    ...
    public int getItemAmountNeeded(int itemid) {
        server.quest.requirements.MapleQuestRequirement req = completeReqs.get(MapleQuestRequirementType.ITEM);
		if(req == null)
			return 0;
    ...
    public int getMobAmountNeeded(int mid) {
        server.quest.requirements.MapleQuestRequirement req = completeReqs.get(MapleQuestRequirementType.MOB);
		if(req == null)
			return 0;
    ...
    public String getInfoEx() {
        server.quest.requirements.MapleQuestRequirement req = startReqs.get(MapleQuestRequirementType.INFO_EX);
		String ret = "";
		if(req != null) {
	...
	private server.quest.requirements.MapleQuestRequirement getRequirement(MapleQuestRequirementType type, MapleData data) {
		server.quest.requirements.MapleQuestRequirement ret = null;
		switch(type) {
			case END_DATE:
	...
	private server.quest.actions.MapleQuestAction getAction(MapleQuestActionType type, MapleData data) {
		server.quest.actions.MapleQuestAction ret = null;
		switch(type) {
			case BUFF:
