...
public class MapleTrade {
    ...
    public static void completeTrade(MapleCharacter c) {
        ...
        if (partner.isLocked()) {
            ...
            if (!local.fitsInInventory() || !partner.fitsInInventory()) {
                cancelTrade(c);
                c.message("Não há espaço no inventário para concluir a troca.");
                partner.getChr().message("Não há espaço no inventário para concluir a troca.");
                return;
            }
            ...
            if (local.getChr().getLevel() < 15) {
                if (local.getChr().getMesosTraded() + local.exchangeMeso > 1000000) {
                    cancelTrade(c);
                    local.getChr().getClient().announce(MaplePacketCreator.serverNotice(1, "Personagens abaixo do nível 15 só podem trocar 1M por dia!"));
                    return;
                } else {
    ...
    public static void startTrade(MapleCharacter c) {
        if (c.getTrade() == null) {
            ...
            c.message("Você já esta em troca.");
        }
    }

    public static void inviteTrade(MapleCharacter c1, MapleCharacter c2) {
        if (c1.gmLevel() == 3 && c2.gmLevel() == 3) {
            c1.getClient().getSession().write(MaplePacketCreator.serverNotice(5, "GameMaster's não podem realizar trocas."));
            return;
        }
        if (c2.getTrade() == null) {
