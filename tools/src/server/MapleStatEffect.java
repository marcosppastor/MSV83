...
public class MapleStatEffect {
    ...
    private short watk, matk, wdef, mdef, acc, avoid, speed, jump, watk2;
    private short hp, mp;
    ...
    private static MapleStatEffect loadFromData(MapleData source, int sourceid, boolean skill, boolean overTime) {
        ...
        ret.watk2 = (short) MapleDataTool.getInt("pdd", source, 0);
        ...
        if (ret.overTime && ret.getSummonMovementType() == null) {
            ...
            addBuffStatPairToListIfNotZero(statups, MapleBuffStat.WATK2, Integer.valueOf(ret.watk2));
        ...
        if (skill) {
            switch (sourceid) {
                ...
                case Buccaneer.SUPER_TRANSFORMATION:
                    statups.add(new Pair<>(MapleBuffStat.MORPH, Integer.valueOf(x)));
                    break;
         ...
         AutobanFactory.MPCON.addPoint(applyfrom.getAutobanManager(), "mpCon hack for skill:" + sourceid + "; Player MP: " + applyto.getMp() + " MP Needed: " + getMpCon());
            ...
            if (!applyto.getWantPvp()) {
                if (hpchange != 0) {
                    ...
                    if (!(applyto.getMapId() == 800020400) && !(applyto.getWantPvp())) {
                        applyto.setHp(newHp);
...
//	final ScheduledFuture<?> schedule = TimerManager.getInstance().schedule(cancelAction, ((starttime + 99999) - System.currentTimeMillis()));
    ...
    private int calcMPChange(MapleCharacter applyfrom, boolean primary) {
        ...
        if (primary) {
            if (mpCon != 0) {
                double mod = 1.0;
                boolean isAFpMage = applyfrom.getJob().isA(MapleJob.MAGOFOGOEVENENO);
                boolean isCygnus = applyfrom.getJob().isA(MapleJob.BLAZEWIZARD2);
                if (isAFpMage || isCygnus || applyfrom.getJob().isA(MapleJob.MAGOGELOERAIO)) {
    ...
    private int alchemistModifyVal(MapleCharacter chr, int val, boolean withX) {
        if (!skill && (chr.getJob().isA(MapleJob.ANDARILHO) || chr.getJob().isA(MapleJob.NIGHTWALKER3))) {
