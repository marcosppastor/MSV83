...
public final class AllianceOperationHandler extends AbstractMaplePacketHandler {
    ...
    public final void handlePacket(SeekableLittleEndianAccessor slea, MapleClient c) {
        ...
        if (alliance == null) {
            c.getPlayer().dropMessage("Você não esta em uma aliança.");
        ...
        switch (slea.readByte()) {
            ...
            case 0x03: // send alliance invite
                ...
                if (channel == -1) {
                    c.getPlayer().dropMessage("O jogador não esta online.");
                } else {
                    ...
                    if (victim.getGuildId() == 0) {
                        c.getPlayer().dropMessage("O jogador escolhido não possui um clã.");
                    } else if (victim.getGuildRank() != 1) {
                        c.getPlayer().dropMessage("O jogador não é o lider do clã.");
                    } else {
...
//                    slea.readMapleAsciiString();//guild name
            ...
            case 0x0A:
                ...
                break;
            default:
                c.getPlayer().dropMessage("Opcão não disponivel.");
        }
        alliance.saveToDB();
    }
