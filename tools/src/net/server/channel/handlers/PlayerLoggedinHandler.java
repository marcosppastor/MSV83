...
public final class PlayerLoggedinHandler extends AbstractMaplePacketHandler {
    ...
    public final void handlePacket(SeekableLittleEndianAccessor slea, MapleClient c) {
        ...
        if (player.getBossLog("BonificaçãoDiária") < 1 && player.getLevel() >= 50) {
            player.dropMessage(6, "[Recompensa Diária] Obtenha sua recompensa diária falando com o Donações!");
        }

        if (player.getCashLog("BonificaçãoCash") < 1 && player.getLevel() >= 70) {
        ...
        /*if (newcomer) {
            if (!c.hasVotedAlready()) {
                //player.announce(MaplePacketCreator.earnTitleMessage("VocÃª jÃ¡ pode votar! Votando com seu personagem deslogado, vocÃª ganha 400 de CashNX."));
            }
