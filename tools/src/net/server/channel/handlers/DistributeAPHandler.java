...
public final class DistributeAPHandler extends AbstractMaplePacketHandler {
    ...
    static int addHP(MapleClient c) {
        ...
        if (job.isA(MapleJob.GUERREIRO) || job.isA(MapleJob.GUERREIRODAAURORA1)) {
            Skill increaseHP = SkillFactory.getSkill(job.isA(MapleJob.GUERREIRODAAURORA1) ? DawnWarrior.MAX_HP_INCREASE : Warrior.IMPROVED_MAXHP);
