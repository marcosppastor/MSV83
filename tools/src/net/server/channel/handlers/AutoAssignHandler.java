...
public class AutoAssignHandler extends AbstractMaplePacketHandler {
    ...
    public void handlePacket(SeekableLittleEndianAccessor slea, MapleClient c) {
        ...
        if (slea.available() < 16) {
            AutobanFactory.EDICAO_PACOTE.autoban(chr, "Não foi enviado um pacote completo para o 'Auto Distribuir'.");
            FilePrinter.printError(FilePrinter.JOGADOR_BANIDO + FilePrinter.EXPLOITS + "", "Em suma, " + chr + " tentou editar um pacote durante a distribuiÃ§Ã£o de AP.\r\n");
            Server.getInstance().broadcastGMMessage(MaplePacketCreator.sendYellowTip("O jogador(a) " + chr + " acaba de ser banido(a) automaticamente pelo seguinte motivo: EDICAO_PACOTE"));
