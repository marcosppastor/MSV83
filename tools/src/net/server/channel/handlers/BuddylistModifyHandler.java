    ...
    public void handlePacket(SeekableLittleEndianAccessor slea, MapleClient c) {
        ...
        if (mode == 1) { // add
            ...
            if (ble != null && !ble.isVisible() && group.equals(ble.getGroup())) {
                c.announce(MaplePacketCreator.serverNotice(1, "Você já tem \"" + ble.getName() + "\" em sua 'Lista de Amigos'"));
            } else if (buddylist.isFull() && ble == null) {
                c.announce(MaplePacketCreator.serverNotice(1, "Sua 'Lista de Amigos' está cheia"));
            } else if (ble == null) {
                try {
                    ...
                    if (charWithId != null) {
                        ...
                        if (buddyAddResult == BuddyAddResult.BUDDYLIST_FULL) {
                            c.announce(MaplePacketCreator.serverNotice(1, "\"" + addName + "\"' está com a Lista de Amigos cheia"));
                        } else {
                            int displayChannel;
                            displayChannel = -1;
                        ...
                        c.announce(MaplePacketCreator.serverNotice(1, "O personagem inserido \"" + addName + "\" não existe"));
                    }
            ...
            if (!buddylist.isFull()) {
                try {
                    ...
                    if (otherName != null) {
                        buddylist.put(new BuddylistEntry(otherName, "Grupo Padrão", otherCid, channel, true));
                        c.announce(MaplePacketCreator.updateBuddylist(buddylist.getBuddies()));
                        notifyRemoteChannel(c, channel, otherCid, ADDED);
                    }
                } catch (SQLException e) {
