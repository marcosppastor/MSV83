...
public final class GiveFameHandler extends AbstractMaplePacketHandler {
    ...
    public final void handlePacket(SeekableLittleEndianAccessor slea, MapleClient c) {
        ...
        if (target == null || target.getId() == player.getId() || player.getLevel() < 15) {
            ...
            AutobanFactory.EDICAO_PACOTE.autoban(jogador, "O jogador(a) " + c.getPlayer().getName() + " tentou alterar algo referente aos pacotes da fama.");
            FilePrinter.printError(FilePrinter.JOGADOR_BANIDO + FilePrinter.EXPLOITS + "", "Em suma, " + c.getPlayer().getName() + " tentou alterar algo referente aos pacotes de fama " + famechange + "\r\n");
            Server.getInstance().broadcastGMMessage(MaplePacketCreator.sendYellowTip("O jogador(a) " + c.getPlayer().getName() + " acaba de ser banido(a) automaticamente por EDICAO_PACOTE de famar."));
            jogador.dropMessage(0, "Você acaba de ser banido(a) automaticamente por EDICAO_PACOTE de fama.");
