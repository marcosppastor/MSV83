...
public class MapleServerHandler extends IoHandlerAdapter {
    ...
    public void sessionOpened(IoSession session) {
        ...
        if (Funções.PROTECAO_DDOS) {
            for (String proxy : Funções.PROXY_IPS) {
                if (!session.getRemoteAddress().toString().contains(proxy)) {
                    System.out.println("IP não identificado tentando ir GameServer - " + session.getRemoteAddress());
                    FilePrinter.printError(FilePrinter.SESSAO, "IP não identificado tentando ir GameServer " + session.getRemoteAddress() + " as " + sdf2.format(Calendar.getInstance().getTime()) + " no dia " + sdf.format(Calendar.getInstance().getTime()) + ", às " + sdf2.format(Calendar.getInstance().getTime()) + ".\r\n");
                    session.close();
                    return;
                }
            }
