...
import server.life.SpawnPointPosition;
...
public class MapleMap {
    ...
    private List<SpawnPointPosition> monsterSpawnPosition = Collections.synchronizedList(new LinkedList<SpawnPointPosition>());
        ...
        for (final MonsterDropEntry de : dropEntry) {
            if (Randomizer.nextInt(999999) < de.chance * chServerrate) {
                ...
                if (de.itemId == 0) { // meso
                    ...
                    FilePrinter.printError(FilePrinter.COMANDOS_GM + FilePrinter.COMANDO_USADO + "", idrop + " foi dropado do mob_id " + mob.getId() + ", em " + mapid + ".\r\n");
    ...
    public void addPlayer(final MapleCharacter chr) {
        ...
        chr.announce(MaplePacketCreator.earnTitleMessage("Você está em " + getMapName()));
    ...
    public void addMonsterSpawnPosition(int monster, Point pos, int mobTime, int team, short fh, short rx0, short rx1, short cy, short f) {
        Point newpos = calcPointBelow(pos);
        if (newpos == null) {
            return;//nothing
        }
        newpos.y -= 1;
        SpawnPointPosition sp = new SpawnPointPosition(monster, newpos, mobTime, team, fh, rx0, rx1, cy, f);
        monsterSpawnPosition.add(sp);
        if (sp.shouldSpawn() || mobTime == -1) {// -1 does not respawn and should not either but force ONE spawn
            sp.spawnMonster(this);
        }
    }
