...
public class MapleMonster extends AbstractLoadedMapleLife {
    ...
    /*
     public void giveExpToCharacternavio(MapleCharacter attacker, int exp, boolean isKiller, int numExpSharers) {
     if (isKiller) {
            ...
            if (killer.getParty() != null) {
                for (MaplePartyCharacter mpc : killer.getParty().getMembers()) {
                    if (mpc.getMapid() == killer.getMapId()) {
                        mpc.getPlayer().finishAchievement(recompensa);
                    }
                }
            } else {
                killer.finishAchievement(recompensa);
            }
        }
    ...
    public boolean applyStatus(MapleCharacter from, final MonsterStatusEffect status, boolean poison, long duration, boolean venom) {
        ...
        if (poison) {
            ...
            if (from.getJob() == MapleJob.LORDENEGRO || from.getJob() == MapleJob.MESTREDASSOMBRAS || from.getJob().isA(MapleJob.NIGHTWALKER3)) {
    ...
    public final void applyMonsterBuff(final MonsterStatus status, final int x, final int skillId, final long duration, final MobSkill skill) {
        MapleTimer.MonsterTimer timerManager = MapleTimer.MonsterTimer.getInstance();
        final Runnable cancelTask = new Runnable() {
            @Override
            public void run() {
                if (isAlive()) {
    ...
    public final void cancelMonsterBuff(final MonsterStatus status) {
        if (isAlive()) {
            byte[] packet = MaplePacketCreator.cancelMonsterStatus(getObjectId(), Collections.singletonMap(status, Integer.valueOf(1)));
            map.broadcastMessage(packet, getPosition());
            if (getController() != null && !getController().isMapObjectVisible(MapleMonster.this)) {
                getController().getClient().getSession().write(packet);
            }
