...
public class MaplePvp {
    ...
    private static void monsterBomb(MapleCharacter player, MapleCharacter attackedPlayers, MapleMap map, AbstractDealDamageHandler.AttackInfo attack) {
        ...
        player.getClient().getSession().write(MaplePacketCreator.serverNotice(5, "Você acertou o jogador" + attackedPlayers.getName() + "  com " + attackedDamage + " de dano!"));
        ...
        if (attackedPlayers.getHp() <= 0 && !attackedPlayers.isAlive()) {
            ...
            player.getClient().getSession().write(MaplePacketCreator.serverNotice(6, "Você matou " + attackedPlayers.getName() + "!! Você ganhou um fragmento de Medo!"));
