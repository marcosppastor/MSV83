    ...
    public final void handlePacket(SeekableLittleEndianAccessor slea, MapleClient c) {
        ...
        if (mode == Action.CREATE.getCode()) {
            ...
            if (chr.getTrade() != null && chr.getTrade().getPartner() != null) {
                ...
                if (ob instanceof MaplePlayerShop) {
                    ...
                    if (shop.isBanned(chr.getName())) {
                        chr.dropMessage(1, "Você foi banido deste comércio.");
                        return;
                    }
                    ...
                    if (merchant.isOwner(c.getPlayer())) {
                        ...
                        chr.dropMessage(1, "Este comércio está em manutenção. Volte mais tarde.");
                        return;
                    } else if (merchant.getFreeSlot() == -1) {
                        chr.dropMessage(1, "Este comércio encontra-se cheio de visitantes. Aguarde até que um saia.");
                        return;
                    } else {
            ...
            if (chr.getItemQuantity(ivItem.getItemId(), false) < perBundle * bundles) {
                return;
            } else if (Funções.USE_ENFORCE_UNMERCHABLE_PET && ItemConstants.isPet(ivItem.getItemId())) {
                c.announce(MaplePacketCreator.serverNotice(1, "Não é permitida a venda de animais na loja."));
                return;
            }
            ...
            if (shop != null && shop.isOwner(c.getPlayer())) {
                ...
                if (slot >= shop.getItems().size() || slot < 0) {
                    ...
                    jogador.dropMessage(0, "Você acaba a de ser banido(a) automaticamente por EDICAO_PACOTE (Tentou remover um item do seguinte slot: " + slot + ").");
            ...
            if (quantity < 1) {
                ...
                jogador.dropMessage(0, "Você acaba de ser banido(a) automaticamente por EDICAO_PACOTE (Tentou comprar o item " + item + " com a seguinte quantidade: " + quantity + ").");
                c.disconnect(true, false);
                return;
            }
            MaplePlayerShop shop = chr.getPlayerShop();
            ...
            if (merchant != null && merchant.isOwner(c.getPlayer())) {
                ...
                merchant.closeShop(c, false);
                chr.setHasMerchant(false);
                c.getPlayer().mensagemCabeca("Você fechou sua loja. Seus items foram devolvidos ao seu inventário!");
            }
