...
public final class CashOperationHandler extends AbstractMaplePacketHandler {
    ...
    public final void handlePacket(SeekableLittleEndianAccessor slea, MapleClient c) {
        ...
        if (action == 0x03 || action == 0x1E) {
            ...
            c.announce(MaplePacketCreator.showCash(chr));
            try {
                chr.sendNote(recipient.get("name"), chr.getName() + " lhe enviou um presente, cheque o seu CashShop!", (byte) 0); //fame or not
            ...
            if (partner == null) {
                chr.getClient().announce(MaplePacketCreator.serverNotice(1, "O parceiro qual você especificou não foi encontrado.\r\nPor favor tenha a certeza de que o nome especificado encontra-se no mesmo canal e esteja online."));
            } else {
            ...
            if (partner == null) {
                chr.dropMessage("O parceiro qual você especificou não foi encontrado.\\r\\nPor favor tenha a certeza de que o nome especificado encontra-se no mesmo canal e esteja online.");
            } else {
