    ...
    public final void handlePacket(SeekableLittleEndianAccessor slea, MapleClient c) {
        ...
        if (operation == Actions.TOSERVER_SEND_ITEM.getCode()) {
            ...
            if (mesos < 0 || (long) mesos > Integer.MAX_VALUE || ((long) mesos + fee + getFee(mesos)) > Integer.MAX_VALUE || amount < 1) {
                AutobanFactory.EDICAO_PACOTE.autoban(jogador, "O jogador(a) " + c.getPlayer().getName() + " tentou editar pacote no Duey.");
                FilePrinter.printError(FilePrinter.JOGADOR_BANIDO + FilePrinter.EXPLOITS + "", "Em suma, " + c.getPlayer().getName() + " tentou usar o Duey com " + mesos + " mesos com a seguinte quantidade:" + amount + "\r\n");
                Server.getInstance().broadcastGMMessage(MaplePacketCreator.sendYellowTip("O jogador(a) " + c.getPlayer().getName() + " acaba de ser banido(a) automaticamente por EDIÃ‡ÃƒO_PACOTE no Duey."));
                jogador.dropMessage(0, "Você acaba de ser banido(a) automaticamente por EDICAO_PACOTE no Duey.");
