    ...
    public final void handlePacket(SeekableLittleEndianAccessor slea, MapleClient c) {
        ...
        if (itemType == 505) { // AP/SP reset
                ...
                switch (APFrom) {
                    ...
                    case 2048: // HP
                        ...
                        if (jobid == 0 || jobid == 1000 || jobid == 2000 || jobid >= 1200 && jobid <= 1211) { // Beginner
                            ...
                            hplose -= 17;
                        } else if (jobid >= 300 && jobid <= 322 || jobid >= 400 && jobid <= 422) { // Aran
                            hplose -= 20;

                        } else if (jobid >= 2000 && jobid <= 2112) { // Aran
                            hplose += 34;
                    ...
                    case 8192: // MP
                        ...
                        if (job.isA(MapleJob.Lanceiro) && mp < 4 * level + 156) {
                            canWash = false;
                        } else if (job.isA(MapleJob.SOLDADO) && mp < 4 * level + 56) {
                            canWash = false;
                        } else if (job.isA(MapleJob.GATUNO) && job.getId() % 100 > 0 && mp < level * 14 - 4) {
                            canWash = true;
                            ;
                        } else if (job.isA(MapleJob.ARAN1) && job.getId() % 100 > 0 && mp > level * 4 + 105) {
                            canWash = true;
                            ;
                        ...
                        if (canWash) {
                            int minmp = 0;
                            if (job.isA(MapleJob.GUERREIRO) || job.isA(MapleJob.GUERREIRODAAURORA1) || job.isA(MapleJob.ARAN1)) {
                                minmp += 4;
                            } else if (job.isA(MapleJob.Bruxo) || job.isA(MapleJob.BLAZEWIZARD1)) {
                                minmp += 36;
                            } else if (job.isA(MapleJob.ARQUEIRO) || job.isA(MapleJob.WINDARCHER1) || job.isA(MapleJob.GATUNO) || job.isA(MapleJob.NIGHTWALKER1)) {
                                minmp += 12;
            ...
            if (itemId == 5060000) { // Item tag.
                ...
                if (Incubadora.getIncubatedItem(c, item2.getItemId())) {
                    remove(c, itemId);
            ...
            MapleInventoryManipulator.removeById(c, MapleInventoryType.CASH, 5060002, 1, true, false);
            ...
            switch (itemId / 1000 % 10) {
                case 1: // Megaphone
                    if (player.getLevel() > 9) {
                        ...
                        player.dropMessage(1, "Requer-se LV. 10 ou superior para poder usar este item.");
                    }
                    break;
                ...
                case 5: // Maple TV
                    ...
                    if (!MapleTVEffect.isActive()) {
                        ...
                        player.dropMessage(1, "O MapleTV já esta em uso.");
                        return;
                    }
                    break;
                case 6: //item megaphone
                    ...
                    if (slea.readByte() == 1) { //item
                        ...
                        if (item == null) //hack
                        {
                            ...
                            player.dropMessage(1, "Você não pode trocar este item.");
                            c.announce(MaplePacketCreator.enableActions());
                            return;
                        }
                    }
            ...
            String error1 = "O outro jogador esta num local inacessivel, ou você não pode ir até um mapa ilegal.";
            ...
            if (!vip) {
                ...
                if (victim != null) {
                    ...
                    if (c.getChannelServer().getMapFactory().getMap(victim.getMapId()).getForcedReturnId() == 999999999 || victim.getMapId() < 100000000) {
                        ...
                        player.dropMessage(1, "Você não pode se teleportar para este mapa.");
                    }
                } else {
                    player.dropMessage(1, "Jogador não se encontra neste canal.");
                }
                if (!success) {
                    MapleInventoryManipulator.addById(c, itemId, (short) 1);
                    c.announce(MaplePacketCreator.enableActions());
