...
public class World {
    ...
    public void buddyChanged(int cid, int cidFrom, String name, int channel, BuddyOperation operation) {
        ...
        if (addChar != null) {
            ...
            switch (operation) {
                case ADDED:
                    if (buddylist.contains(cidFrom)) {
                        buddylist.put(new BuddylistEntry(name, "Grupo Padrão", cidFrom, channel, true));
                        addChar.getClient().announce(MaplePacketCreator.updateBuddyChannel(cidFrom, (byte) (channel - 1)));
                    }
                    break;
                case DELETED:
                    if (buddylist.contains(cidFrom)) {
                        buddylist.put(new BuddylistEntry(name, "Grupo Padrão", cidFrom, (byte) -1, buddylist.get(cidFrom).isVisible()));
    ...
    public void setServerMensagem(String msg) {
        for (Channel ch : channels) {
            ch.setServerMensagem(msg);
        }
    }
