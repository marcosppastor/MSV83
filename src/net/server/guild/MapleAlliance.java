...
public class MapleAlliance {
    ...
    public MapleAlliance(String name, int id, int guild1, int guild2) {
        ...
        String[] ranks = {"Mestre", "Mestre Jr.", "Membro", "Membro", "Membro"};
        for (int i = 0; i < 5; i++) {
            guilds[i] = guild[i];
    ...
    public static MapleAlliance createAlliance(MapleCharacter chr1, MapleCharacter chr2, String name) {
        ...
        try {
            PreparedStatement ps = DatabaseConnection.getConnection().prepareStatement("INSERT INTO `alliance` (`name`, `guild1`, `guild2`) VALUES (?, ?, ?)", Statement.RETURN_GENERATED_KEYS);
            ps.setString(1, name);
            ps.setInt(2, guild1);
            ps.setInt(3, guild2);
            ps.executeUpdate();
