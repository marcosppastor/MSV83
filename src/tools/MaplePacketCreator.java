...
import server.lojas.AbstractPlayerStore;
import server.lojas.IMaplePlayerShop;
import server.MapleShopItem;
import server.maps.AbstractMapleMapObject;
import server.maps.*;
import server.movement.LifeMovementFragment;
import static tools.KoreanDateUtil.getItemTimestamp;
...
public class MaplePacketCreator {
    ...
    public static byte[] getAuthSuccess(MapleClient c) {
        ...
        mplew.write(Funções.ATIVAR_PIN ? 0 : 1); // 0 = Pin-System Enabled, 1 = Disabled
        mplew.write(Funções.ATIVAR_PIC ? (c.getPic() == null ? 0 : 1) : 2); // 0 = Register PIC, 1 = Ask for PIC, 2 = Disabled
    ...
    public static byte[] pinRegistered3() {
        final MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter(3);
        mplew.writeShort(SendOpcode.UPDATE_PINCODE.getValue());
        mplew.write(0);
        return mplew.getPacket();
    ...
    public static byte[] pinRegistered2() {
        final MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter(3);
        mplew.writeShort(SendOpcode.UPDATE_PINCODE.getValue());
        mplew.write(0);
        return mplew.getPacket();
    ...
    public static byte[] getCharList(MapleClient c, int serverId) {
        ...
        mplew.write(Funções.ATIVAR_PIC ? (c.getPic() == null ? 0 : 1) : 2);
     ...
     */
    public static MaplePacket serverMensagem(String mensagem) {
        return serverMensagem(4, 0, mensagem, true, false);
    }
     ...
     * @return The server notice packet.
     */
    private static MaplePacket serverMensagem(int type, int channel, String mensagem, boolean servermessage, boolean megaEar) {
        ...
        mplew.writeMapleAsciiString(mensagem);
        if (type == 3) {
    ...
    public static byte[] gachaponMessage(Item item, String town, MapleCharacter player) {
        ...
        mplew.writeShort(SendOpcode.SERVERMESSAGE.getValue());
        mplew.write(0x0B);
        mplew.writeMapleAsciiString(player.getName() + " : obteu um(a)");
     ...
     * @return The item gain packet.
     */
    public static MaplePacket getMostrarItemGanho(int itemId, short quantity, boolean inChat) {
        MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();
        if (inChat) {
    ...
    public static byte[] spawnPlayerMapobjectcpq(MapleCharacter chr) {
        MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();
        mplew.writeShort(SendOpcode.SPAWN_PLAYER.getValue());
        mplew.writeInt(chr.getId());
        mplew.writeMapleAsciiString(chr.getName());
        ...
        if (ipim != null && ipim.isOwner(chr)) {
            if (ipim.getShopType() == 3 || ipim.getShopType() == 4) {
                //addAnnounceBox(mplew, ipim);
            }
        } else if (chr.getMiniGame() != null && chr.getMiniGame().isOwner(chr)) {
            if (chr.getMiniGame().hasdrieSlot()) {
                addAnnounceBox(mplew, chr.getMiniGame(), 1, 0, 1, 0);
            } else {
                addAnnounceBox(mplew, chr.getMiniGame(), 1, 0, 2, 1);
            }
    ...
    public static void addAnnounceBox(MaplePacketLittleEndianWriter mplew, MapleCharacter chr) {
        if ((chr.getlojaJogador() != null) && (chr.getlojaJogador().isOwner(chr)) && (chr.getlojaJogador().getShopType() != 1) && (chr.getlojaJogador().isAvailable())) {
            addInteraction(mplew, chr.getlojaJogador());
        } else {
            mplew.write(0);
        }
    }
     ...
     * 03 = Por favor, verifique se o seu inventario está cheio ou não.
    ...
    public static byte[] damagePlayerpvp(int skill, int monsteridfrom, int cid, int damage) {
        return damagePlayer(skill, monsteridfrom, cid, damage, 0, 0, false, 0, true, 0, 0, 0);
    }

    public static byte[] damagePlayerpvp(int skill, int monsteridfrom, int cid, int damage, int fake, int direction, boolean pgmr, int pgmr_1, boolean is_pg, int oid, int pos_x, int pos_y) {
        MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();
    ...
    public static byte[] removeSpecialMapObject(MapleCharacter chr, int skill, boolean animated) {
        MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();
        mplew.writeShort(SendOpcode.REMOVE_SPECIAL_MAPOBJECT.getValue());
    ...
    public static byte[] boatPacketFree(boolean type) {
        final MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();
        mplew.writeShort(SendOpcode.BOAT_EFFECT.getValue());
        mplew.write(type ? (byte) 12 : (byte) 8);
        mplew.write(type ? (byte) 6 : (byte) 2);
        return mplew.getPacket();
    }
    ...
    public static MaplePacket attSlotInventario(MapleInventoryType type, Item item, boolean fromDrop) {
        MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();
    ...
    public static MaplePacket addSlotInventario(MapleInventoryType type, Item item, boolean fromDrop) {
        MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();
    ...
    private static void addItemInfo(MaplePacketLittleEndianWriter mplew, Item item, boolean zeroPosition, boolean leaveOut) {
        MapleItemInformationProvider ii = MapleItemInformationProvider.getInstance();
        boolean ring = false;
        IEquip equip = null;
        ...
        if (masking && !ring) {
            ...
            addExpirarTempo(mplew, item.getExpiration(), false);
        } else if (ring) {
            mplew.writeLong(getKoreanTimestamp((long) (System.currentTimeMillis() * 1.2)));
        } else {
            mplew.writeShort(0);
    ...
    private static void addExpirarTempo(MaplePacketLittleEndianWriter mplew, long time, boolean showexpirationtime) {
        if (time != 0) {
    ...
    public static MaplePacket addInventarioSlot(MapleInventoryType type, Item item) {
        return addSlotInventario(type, item, false);
    }
    ...
    public static MaplePacket addInventorySlot(MapleInventoryType type, Item item) {
        return addInventorySlot(type, item, false);
    }

    public static MaplePacket addInventorySlot(MapleInventoryType type, Item item, boolean fromDrop) {
        MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();
        mplew.writeShort(SendOpcode.INVENTORY_OPERATION.getValue());
        if (fromDrop) {
            mplew.write(1);
    ...
    public static MaplePacket updateInventorySlot(MapleInventoryType type, Item item) {
        return atualizarSlotInvent(type, item, false);
    }

    public static MaplePacket atualizarSlotInvent(MapleInventoryType type, Item item, boolean fromDrop) {
        MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();
        mplew.writeShort(SendOpcode.INVENTORY_OPERATION.getValue());
        if (fromDrop) {
            mplew.write(1);
    ...
    public static byte[] giveForeignDebuff(int cid, long mask, MobSkill skill) {
        MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();
        mplew.writeShort(SendOpcode.GIVE_FOREIGN_BUFF.getValue());
        mplew.writeInt(cid);
        mplew.writeLong(0);
    ...
    public static byte[] applyMonsterStatus(int oid, Map<MonsterStatus, Integer> stats, int skill, boolean monsterSkill, int delay) {
        return applyMonsterStatus(oid, stats, skill, monsterSkill, delay, null);
    }

    public static byte[] updatePet(MaplePet pet) {
        MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();
    ...
    public static byte[] getAuthSuccessRequestPin(String account) {
        MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();
        mplew.writeShort(SendOpcode.LOGIN_STATUS.getValue());
     ...
     * @return The PIN request packet.
     */
    public static byte[] getAuthSuccessRequestPin(MapleClient c) {
        MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();

        /*mplew.writeShort(SendPacketOpcode.LOGIN_STATUS.getValue());
         mplew.write(new byte[]{0, 0, 0, 0, 0, 0, 
         (byte) 0xFF, 0x6A, 1, 0, 
         0, 0, 0x4E});
         mplew.writeMapleAsciiString(account);
         mplew.write(new byte[]{3, 0, 0, 0, 0, 0, 0, 0, 
         0, 0, 
